...
4. Create an account, then assign the administrator role using a trusted database connection / Supabase SQL editor:
...
The unit tests cover quantities, persisted cart validation, upload limits, form validation, storage retries and authentication cookie refresh.
...
- The cart is a local inquiry list. Quantities appear in totals and WhatsApp messages; prices and stock are confirmed by the business.

## Current scope

Available: catalog, login/register/account, product management, slide management, shared company settings and live dashboard counts.

...
export default function ProductsCatalog({ initialQuery, category }: { initialQuery: string; category: string }) {
  const [query, setQuery] = useState(initialQuery);
  const filtered = useMemo(() => products.filter((product) => { const text = `${product.name} ${product.brand} ${product.category}`.toLocaleLowerCase("tr-TR"); return (!query || text.includes(query.toLocaleLowerCase("tr-TR"))) && (!category || product.categorySlug === category); }), [category, query]);

...
export default function ProductCard({ product }: { product: Product }) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-gray-200 bg-white">
      ...
      <div className="mt-5 flex items-center justify-between gap-3"><strong className="text-lg text-gray-900">{formatPrice(product.price)}</strong><div className="flex gap-2"><WhatsAppLink title="WhatsApp ile bilgi al" message={`Merhaba, ${product.name} hakkında bilgi almak istiyorum.`} className="flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 text-[#174d32] hover:bg-[#f3f7e7]"><MessageCircle size={17} /></WhatsAppLink></div></div>
      </div>
    </article>
  );
}

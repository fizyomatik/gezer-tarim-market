...
export default function Hero({ slides }: { slides: Slide[] }) {
  ...
  if (!slide) return <section className="bg-[#174d32] px-5 py-20 text-center text-white"><h1 className="text-4xl font-bold">Gezer Tarım Market</h1><p className="mt-5">Tohum, gübre, tarım aletleri ve servis hizmetleri.</p><Link href="/products" className="mt-6 inline-block underline">Ürünleri incele</Link></section>;
  return (
    <section className="relative overflow-hidden bg-[#174d32]">
      <div className="mx-auto grid min-h-[600px] max-w-7xl items-center gap-10 px-5 py-16 lg:grid-cols-[1fr_0.9fr] lg:px-8">
        ...
        <div className="relative hidden aspect-[4/3] overflow-hidden rounded-3xl border border-white/20 lg:block"><Image key={slide.image} src={slide.image} alt={slide.title} fill sizes="(min-width: 1024px) 45vw, 100vw" className="object-cover" priority={active === 0} /><div className="absolute inset-0 bg-gradient-to-t from-[#174d32]/70 to-transparent" /><div className="absolute bottom-6 left-6 text-sm font-bold text-white">Gezer Tarım Market</div></div>
      </div>

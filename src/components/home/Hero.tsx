...
export default function Hero({ slides }: { slides: Slide[] }) {
  ...
  useEffect(() => { if (slides.length < 2) return; const timer = window.setInterval(() => setActive((current) => (current + 1) % slides.length), 5000); return () => window.clearInterval(timer); }, [slides.length]);
  const slide = slides[active % slides.length];
  if (!slide) return <section className="bg-[#174d32] px-5 py-20 text-center text-white"><h1 className="text-4xl font-bold">{settings.companyName}</h1><p className="mt-5">Tohum, gübre, tarım aletleri ve servis hizmetleri.</p><Link href="/products" className="mt-6 inline-block underline">Ürünleri incele</Link></section>;
  return (
    <section className="relative overflow-hidden bg-[#174d32]">
      <div className="mx-auto grid min-h-[600px] max-w-7xl items-center gap-10 px-5 py-16 lg:grid-cols-[1fr_0.9fr] lg:px-8">
        ...
        <div className="relative aspect-[4/3] overflow-hidden rounded-3xl border border-white/20"><Image key={slide.image} src={slide.image} alt={slide.title} fill sizes="(min-width: 1024px) 45vw, 100vw" className="object-cover" priority={active === 0} /><div className="absolute inset-0 bg-gradient-to-t from-[#174d32]/70 to-transparent" /><div className="absolute bottom-6 left-6 text-sm font-bold text-white">{settings.companyName}</div></div>
      </div>
      <div className="absolute bottom-8 left-5 z-10 flex items-center gap-3 lg:left-1/2"><button onClick={() => setActive((active - 1 + slides.length) % slides.length)} aria-label="Önceki slayt" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/30 text-white hover:bg-white/10"><ArrowLeft size={17} /></button><div className="flex gap-2">{slides.map((item, index) => <button key={item.id} onClick={() => setActive(index)} aria-label={`${index + 1}. slayt`} className={`h-2 rounded-full transition-all ${active === index ? "w-8 bg-[#a5c63b]" : "w-2 bg-white/40"}`} />)}</div><button onClick={() => setActive((active + 1) % slides.length)} aria-label="Sonraki slayt" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/30 text-white hover:bg-white/10"><ArrowRight size={17} /></button></div>
    </section>
  );
}
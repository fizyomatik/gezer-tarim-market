...
export default function CartPage() {
  ...
  return <main className="min-h-[60vh] bg-[#f7f8f4] py-16"><div className="mx-auto max-w-4xl px-5">
    <h1 className="text-4xl font-black text-[#174d32]">Bilgi talep listem</h1>
    {!items.length ? <div className="mt-10 rounded-2xl bg-white p-12 text-center"><p>Listenizde henüz ürün yok.</p><Link href="/products" className="mt-5 inline-block underline">Ürünlere göz at</Link></div> : <>
      <button onClick={clearCart} className="mt-4 text-red-700">Listeyi temizle</button>
      <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_300px]">
        ...
        <aside className="h-fit rounded-2xl bg-[#174d32] p-6 text-white"><p>{cartTotal(items) === null ? "Fiyat bilgisi" : "Tahmini toplam"}</p><p className="mt-2 text-3xl font-black">{formatPrice(cartTotal(items))}</p>
